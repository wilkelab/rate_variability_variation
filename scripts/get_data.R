...
library(dplyr)
library(tidyr)
enzyme_data = read.table(file = "../data/enzyme_dataset/jec_ddg_209_monomers.csv", sep = ",", header = T)
...
mono_data <- read.table("../data/enzyme_dataset/profiles_213_monomers.csv", sep  = ",", header = T)
...
write.csv(final_enzyme_data, "../data/output_files/enzyme_data.csv")

entropy_data = read.table("../data/enzyme_dataset/pdb_entropies.csv", sep = ",", header = T)
...
write.csv(reduced_entropy_data, "../data/output_files/enzyme_reduced_entropy_data.csv")

combo_enzyme_data = left_join(final_enzyme_data, reduced_entropy_data)
write.csv(combo_enzyme_data, "../data/output_files/all_enzyme_data.csv")

virus_rate_data = read.table("../data/virus_protein_evolutionary_rates.csv" , sep = ",", header = T)

virus_rate_data = virus_rate_data %>% filter(pdb != "3I5K")
virus_rate_data = virus_rate_data %>% filter(pdb != "1RD8")
virus_out_data = cbind(virus_out_data, virus_rate_data$r4s_JTT)
...
dengue_pdb = read.table("../data/virus_dataset/data_2JLY_A.csv", sep = ",", header = T) #2JLY
...
dengue_rates = virus_rate_data %>% filter(pdb == "2JLY")
v_rates = dengue_rates %>% select(pdb, chain, r4s_JTT)
dengue_pdb = cbind(dengue_pdb, v_rates)



west_nile_pdb = read.table("../data/virus_dataset/data_2FP7_B.csv", sep = ",",  header = T) #2FP7
...
west_nile_rates = virus_rate_data %>% filter(pdb == "2FP7")
v_rates = west_nile_rates %>% select(pdb, chain, r4s_JTT)
west_nile_pdb = cbind(west_nile_pdb, v_rates)



jap_ence_pdb = read.table("../data/virus_dataset/data_2Z83_A.csv", sep = ",", header = T) #2Z83
...
jap_ence_rates = virus_rate_data %>% filter(pdb == "2Z83")
v_rates = jap_ence_rate_rates %>% select(pdb, chain, r4s_JTT)
jap_ence_pdb = cbind(jap_ence_pdb, v_rates)



hep_c_pdb = read.table("../data/virus_dataset/data_3GOL_A.csv", sep = ",", header = T) #3GOL
...
hep_c_rates = virus_rate_data %>% filter(pdb == "3GOL")
v_rates = hep_c_rates %>% select(pdb, chain, r4s_JTT)
hep_c_pdb = cbind(hep_c_pdb, v_rates)


rift_valley_pdb = read.table("../data/virus_dataset/data_3LYF_A.csv", sep = ",",  header = T) #3LYF
...
rift_valley_rates = virus_rate_data %>% filter(pdb == "3LYF")
v_rates = rift_valley_rates %>% select(pdb, chain, r4s_JTT)
rift_valley_pdb = cbind(rift_valley_pdb, v_rates)



crim_congo_pdb = read.table("../data/virus_dataset/data_4AQF_B.csv",  sep = ",", header = T) #4AQF
...
crim_congo_rates = virus_rate_data %>% filter(pdb == "4AQF")
v_rates = crim_congo_rates %>% select(pdb, chain, r4s_JTT)
crim_congo_pdb = cbind(crim_congo_pdb, v_rates)


marg_bd_pdb = read.table("../data/virus_dataset/data_4GHA_A.csv", sep = ",",  header = T) #4GHA
...
marg_bd_rates = virus_rate_data %>% filter(pdb == "4GHA")
v_rates = marg_bd_rates %>% select(pdb, chain, r4s_JTT)
marg_bd_pdb = cbind(marg_bd_pdb, v_rates)


flu_np_pdb = read.table("../data/virus_dataset/data_4IRY_A.csv", sep = ",", header = T ) # 4IRY
...
flu_rates = virus_rate_data %>% filter(pdb == "4IRY")
v_rates = flu_rates %>% select(pdb, chain, r4s_JTT)
flu_pdb = cbind(flu_pdb, v_rates)
...
write.csv(virus_out_data, "../data/output_files/sharamoradi_et_al_data.csv")

all_data <- rbind(combo_enzyme_data, virus_out_data) 
write.csv(all_data, "../data/output_files/all_pdb_data.csv")
