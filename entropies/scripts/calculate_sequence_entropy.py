...
def calculate_aa_frequencies(list_of_aligned_seqs): 
	...
	for seq in list_of_aligned_seqs:
		aligned_seqs.append(ph.get_sequence_array(seq)) #Turn the lists of sequences into a list of arrays (where each array represents a sequence 
...
def main(argv = sys.argv):
	...
	if len( sys.argv ) != 3:
		...
		aligned_seqs, headers = ph.get_sequences(alignment_file)
		...
		for site in aa_freqs: #Calculate the frequencies of each amino acid of each site in the alignment
			freq_list.append(ph.get_sequence_array(site))
